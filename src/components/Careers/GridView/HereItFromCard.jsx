...
export default function HereItFromCard({ id, type, src, text, author, color, location }) {
  const isQuote = type === "quote";
  const isVideo = type === "video";

  return (
<div className={`${styles.careersBox} ${styles[`careersBox${id}`]} ${isQuote ? styles[`bg${color}`] : ""}`}>
  {isVideo && (
    <>
      <video className={styles.careersVideoBg} autoPlay muted loop playsInline>
